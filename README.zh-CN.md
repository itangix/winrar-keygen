...
+ [通过 zig build 编译使用](#6-通过-zig-build-编译使用)
+ [通过 Github Actions 编译使用](#7-通过-Github-Actions-编译使用)
+ [通过 zig build-exe 编译使用](#8-通过-zig-build-exe-编译使用)
...
如果你不想自行编译，可以在 [Actions](#7-通过-Github-Actions-编译使用) 中构建，也可以到 [Release](https://github.com/bitcookies/winrar-keygen/releases/) 页面获取对应版本的构建程序。

推荐使用这种方式构建。它由 `build.zig` 驱动，只需要 [Zig](https://ziglang.org/) 编译器（0.16.0 或更高版本）。所有密码学与大整数代码都来自 Zig 标准库，因此没有 GMP 之类的第三方依赖。

### 6.1 前提条件

从 [下载页面](https://ziglang.org/download/) 安装 Zig 0.16.0 或更高版本。

### 6.2 编译

```shell
# Debug 构建 -> zig-out/bin/winrar-keygen
zig build
...
产物位于 `zig-out/bin/winrar-keygen`（Windows 上为 `winrar-keygen.exe`）。也可以一步完成构建并运行：
...
你可以在 Actions 中使用 `zig build` 构建 Linux x64、Linux ARM64、macOS ARM64、Windows x64 和 Windows ARM64 五种架构的应用程序。

点击该项目右上角的 **Fork** 按钮，fork 一份代码到你的 Github。然后找到你 fork 的仓库，进入 Actions 页面并手动执行 **Build (build.yml)**。
...
## 8. 通过 zig build-exe 编译使用

<details>
<summary>点击展开</summary>

如果你不想使用构建脚本，可以直接用 `zig build-exe` 编译（无需 `build.zig`）：

```shell
zig build-exe src/main.zig -OReleaseFast --name winrar-keygen
```

该命令会在当前目录生成 `winrar-keygen`（Windows 上为 `winrar-keygen.exe`）。加上 `-Dtarget=<triple>` 即可交叉编译，例如：

```shell
zig build-exe src/main.zig -OReleaseFast --name winrar-keygen -Dtarget=x86_64-windows-gnu
```

版本号由 `src/main.zig` 中的 `APP_VERSION` 定义。
...
如果你 `ansi` 编码时发生了错误，原因是因为字符编码不受当前 Windows 系统支持。GitHub Actions 的系统平台是 `windows-2025-english` （ANSI code page 1252），包含非 ASCII 字符都会失败。
