...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Zig-0.16.0-F7A41D?logo=zig&logoColor=white" />
...
+ [Build with zig build](#6-Build-with-zig-build)
+ [Build in Github Actions](#7-Build-in-Github-Actions)
+ [Build with zig build-exe](#8-Build-with-zig-build-exe)
...
## 6. Build with `zig build`

<details>
<summary>Click to expand</summary>

If you don't want to compile it yourself, you can build it with [Actions](#7-Build-in-Github-Actions) or download a pre-built version from the [Release](https://github.com/bitcookies/winrar-keygen/releases/) page.

This is the recommended way to build. It is driven by `build.zig` and only needs the [Zig](https://ziglang.org/) compiler (0.16.0 or newer). All cryptography and big-integer code comes from the Zig standard library, so there is no third-party dependency such as GMP.

### 6.1 Prerequisites

Install Zig 0.16.0 or newer from the [download page](https://ziglang.org/download/).

### 6.2 Build

```shell
# Debug build -> zig-out/bin/winrar-keygen
zig build
...
The binary is written to `zig-out/bin/winrar-keygen` (`winrar-keygen.exe` on Windows). You can also build and run it in a single step:

```shell
zig build run -- "Github" "Single PC usage license" -t
```

To cross-compile for another platform, pass `-Dtarget`:
...
In Actions, you can build applications for five architectures: Linux x64, Linux ARM64, macOS ARM64, Windows x64, and Windows ARM64, all with `zig build`.

Click the **Fork** button in the upper-right corner of the project to fork the code to your GitHub. Then locate the forked repository, go to the Actions page, and run **"Build" (build.yml)**.
...
## 8. Build with `zig build-exe`

<details>
<summary>Click to expand</summary>

If you prefer not to use the build script, you can compile directly with `zig build-exe` (no `build.zig` required):

```shell
zig build-exe src/main.zig -OReleaseFast --name winrar-keygen
```

This creates `winrar-keygen` (`winrar-keygen.exe` on Windows) in the current directory. Add `-Dtarget=<triple>` to cross-compile, for example:

```shell
zig build-exe src/main.zig -OReleaseFast --name winrar-keygen -Dtarget=x86_64-windows-gnu
```

The version number is defined by `APP_VERSION` in `src/main.zig`.
...
If you encounter an error while using `ansi` encoding, it is because the character encoding is not supported by the current Windows system. The system platform for GitHub Actions is `windows-2025-english` (ANSI code page 1252), and any non-ASCII characters will cause the process to fail.
